...
class LSTMNet(Layer):
    ...
    def __init__(self, in_dem: int, out_dem: int, learning_distance: int = 0,
        ...
        if self.learning_distance > 0:
            ...
            self.forget_gate = numpy.zeros((self.table_block, self.out_dem))
            self.store_gate = numpy.zeros((self.table_block, self.out_dem))
            self.output_gate = numpy.zeros((self.table_block, self.out_dem))

            self.input_state = numpy.zeros((self.table_block, self.in_dem))
            self.joint_state = numpy.ones((self.table_block, self.joint_dem))

            self.cell_state = numpy.zeros((self.table_block, self.out_dem))
            self.input_tanh = numpy.zeros((self.table_block, self.out_dem))
            self.output_tanh = numpy.zeros((self.table_block, self.out_dem))
